...
import parse, {
	...
	isNum, isAl, validVarChar,
	isOperatorTypeGuard, extractOperator,
	parseArgs, parsePart,
	parseString, parseVar,
	parseImpl,
	parseNum
} from './parse'
import { TExpr, TExpr_object, TExpr_variable } from './types'
...
test('parseImpl', () => {
	...
	expect(Object.keys((parseImpl('  {   abc   :  123  ,  def  }  ', 0, []).expr[0] as TExpr_object).object)).toStrictEqual(['abc','def'])
	expect(Object.keys((parseImpl('  {  "abc"  :  123  ,  def  }  ', 0, []).expr[0] as TExpr_object).object)).toStrictEqual(['abc','def'])
	expect(Object.values((parseImpl('  {  abc  :  123  ,  def  }  ', 0, []).expr[0] as TExpr_object).object)).toStrictEqual([[{type:'number',number:123, subexpr:[]}], []])
