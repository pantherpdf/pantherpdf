...
export default function VarEditor(props: GeneralProps) {
	async function changeVar(idx: number|undefined, name: string|undefined, formula: string|undefined): Promise<void> {
		...
		if (idx !== undefined) {
			if (name || formula) {
				...
				report.variables[idx].formula = formula || report.variables[idx].formula
			}
			else {
	...
	return <div className='mt-3'>
		...
		<table className='table table-sm'>
			...
			<tbody>
				{props.report.variables.map((v, idx) => <tr key={v.name+idx}>
					<td>
						<InputApplyOnEnter
							value={v.name}
				...
				{reservedVars.map(varName => <tr key={varName}>
					<td>
						{varName}
					</td>
					<td></td>
