...
test('generateTarget override', async () => {
	...
	const data = [{abc:"1", def:"2"}, {abc:"3", def:"4"}]
	const api: ApiEndpoints = { }
	const result = await generateTarget({report, api, data:{value:data, type:'as-is'}, targetOverride: 'json', makePdf:()=>{throw new Error('not supported')}})
	expect(result.filename).toBe('abc.json')
	expect(result.contentType).toBe('application/json')
	const txt = new TextDecoder().decode(result.body);
	expect(JSON.parse(txt)).toEqual([["1","2"],["3","4"]])
})
