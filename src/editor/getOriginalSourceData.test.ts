...
test('getOriginalSourceData javascript', async () => {
	...
	const data = await getOriginalSourceData({report, api, data:input, allowUnsafeJsEval: true})
	expect(data).toBe(9)

	// allowUnsafeJsEval default is false, should reject
	await expect(getOriginalSourceData({report, api, data:input})).rejects.toThrow()
})
