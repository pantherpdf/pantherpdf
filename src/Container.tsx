...
export default function Container() {
  ...
  const [overrideSourceData, setOverrideSourceData] = useState<
    string | undefined
  >(undefined);
  ...
  const reportEditable =
    params.get('editable') === null ||
    params.get('editable') === '1' ||
    params.get('editable') === 'true';
  const loadLocalReport =
    params.get('loadLocalReport') === '1' ||
    params.get('loadLocalReport') === 'true';
  ...
  const api: ApiEndpoints = {
    ...
    filesDelete: filesUrl
      ...
      : undefined,
    filesUpload: filesUrl
      ? async (file, data, cbProgress) => {
    ...
    filesDownloadUrl: filesDownloadUrl
      ...
      : undefined,
    filesDownload: filesDownloadUrl
      ? async name => {
          const url = filesDownloadUrl.replace(
            ':name',
          ...
          let mimeType = (r.headers.get('Content-Type') || '')
            .split(';')[0]
            .trim();
    ...
    (async function () {
      try {
        if (loadLocalReport) {
          ...
          const report =
            reportTxt && reportTxt.length > 0 ? JSON.parse(reportTxt) : null;
      ...
      <main className="container">
        <div className="d-flex justify-content-center">
          <FontAwesomeIcon
            icon={faSpinner}
            spin={true}
            style={{ margin: '50px auto', fontSize: '2rem' }}
          />
        </div>
      </main>
    );
