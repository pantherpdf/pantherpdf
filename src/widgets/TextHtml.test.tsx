...
import { TextHtmlData, TextHtmlCompiled, ValueInternalFromEditor, extractTag } from './TextHtml'
...
test('extractTag', () => {
	const { formula, adjust, tags } = extractTag('<data data-adjust="123&gt;8"><i></i><div><i></i><span data-abc="123" def>456</span></div></data>')
...
test('TextHtml 2', async () => {
	const dt: TextHtmlData = {type:'TextHtml', value:ValueInternalFromEditor('Hello <data><i></i><b>data.txt</b></data>'), children:[], font:{}}
	const data = { txt: '123' }
...
test('TextHtml Filter', async () => {
	const dt: TextHtmlData = {type:'TextHtml', value:ValueInternalFromEditor('Hello <data data-adjust="num, 2 dec">data.num</data>'), children:[], font:{}}
...
test('TextHtml should render html', async () => {
	const report: ReportForceChildren<TextHtmlData> = {
		...sampleReport,
		children: [
			{type:'TextHtml', value:ValueInternalFromEditor('Hello <b>world</b>'), children:[], font:{size: '20px'}}
		]
	}
...
test('TextHtml should include google font', async () => {
	const report: ReportForceChildren<TextHtmlData> = {
		...sampleReport,
		children: [
			{type:'TextHtml', value:ValueInternalFromEditor('text'), children:[], font:{size: '20px', family: 'Lato'}}
		]
	}
	const compiled = await compile(report, {})
